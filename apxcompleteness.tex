...
\begin{proof}
  ...
  \begin{align*}
    ...
    c_Q(\phi) & = |\phi| \\
    \alpha(n + M) & = 1 \\
    \beta(n + M) & = 2^{k + 1} \cdot (k - 2) - 1 \qquad \text{(see explanation below)},
  \end{align*}
  ...
  Since $c_Q(f(\pair{U}{F})) = |f_0(\pair{U}{F})| \leq |F| \cdot 2^k \cdot (k - 2)$, we must choose $\beta$ such that $\frac{1}{2} |F| \leq \frac{1}{1 + \beta(|f(\pair{U}{F}))|)} \cdot |F| \cdot 2^k \cdot (k - 2)$.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \tilde{\epsilon} & = \beta(|f'(f(\phi))|) \\
    & = 2^{k + 1} \cdot (k - 2) - 1. \qedhere
  \end{align*}
  \begin{todo}
    Need to change the definitions, since $\tilde{\epsilon}$ here needs access to the length of $f'(f(x))$.
  \end{todo}
\end{proof}
