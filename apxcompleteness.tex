...
\begin{lemma}\label{lem:opt}
  There exists a polynomial time reduction of some kind (???) from \textsc{Maximum $k$-Function Satisfiability} to \textsc{Maximum 3-Satisfiability}, with preservation factor $\alpha$ defined by $\alpha = 2^{k + 1} \cdot (k - 2) - 1$.
\end{lemma}
\begin{proof}
