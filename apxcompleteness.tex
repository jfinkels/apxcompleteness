%% apxcompleteness.tex - hardness of approximation for MAX 3-SAT
%%
...
\hypersetup{pdftitle={Revisiting the hardness of approximation of Maximum 3-Satisfiability}, pdfauthor={Jeffrey Finkelstein}}

%% Declare the bibliography file.
\addbibresource{apxcompleteness.bib}

\declaretheorem[numberwithin=section]{theorem}
\declaretheorem[numberlike=theorem]{corollary}
\declaretheorem[numberlike=theorem]{lemma}
\declaretheorem[numberlike=theorem, name=TODO]{todo}
\declaretheorem[style=definition, numberlike=theorem]{definition}
...
\symbolfootnote{%
  Copyright 2012, 2014 Jeffrey~Finkelstein \email{jeffreyf@bu.edu}.
  ...
  The \LaTeX{} markup that generated this document can be downloaded from its website at \mbox{\url{https://github.com/jfinkels/apxcompleteness}}.
...
A function $f \colon I \times \mathbb{N} \to \Sigma^*$ is an \emph{approximation scheme} if $R(x, f(x, k)) \leq 1 + \frac{1}{k}$ for all $x \in I$ and all $k \in \mathbb{N}$.
...
\subsubsection{Classes of approximable optimization problems}

We will also consider classes of optimization problems which have efficient approximation algorithms.
\begin{definition}
  Let $P \in \NPO$.
  $P \in \APX$ if there exists a polynomial time computable $r$-approximator for $P$, where $r(n) \in O(1)$ for all $n \in \mathbb{N}$.
  $P \in \PTAS$ if there exists an approximation scheme $f$ for $P$ such that $f(x, k)$ is computable in polynomial time with respect to the length of $x$.
\end{definition}
\begin{definition}
  Let $P \in \NNCO$.
  $P \in \NCX$ if there exists an $r$-approximator in \FNC{} for $P$, where $r(n) \in O(1)$ for all $n \in \mathbb{N}$.
  $P \in \NCAS$ if there exists an approximation scheme $f$ for $P$ such that $f_k \in \FNC$ for each $k \in \mathbb{N}$, where $f_k(x) = f(x, k)$ for all $x \in \Sigma^*$.
\end{definition}
...
\end{lemma}
\begin{proofidea}
  Iteratively choose the true or false setting of the variable which would satisfy the greatest number of clauses, simplifying after each step.
\end{proofidea}
...
\begin{definition}[{{\cite[Section~29.1]{vazirani}}}]\label{def:intro}
  Let $P$ be a decision problem and $Q$ be a maximization problem with $Q = (I, S, m, \max)$.
...
%% Notice from \autoref{lem:xinp} that when an enhanced gap-introducing reduction has ``reasonable'' parameters, it is vacuously true that $\left(x, g(x, y)\right) \in R_P$ implies $x \in P$, since the conclusion is always true.
...
%%   Suppose there is a gap-introducing reduction $(f, c, \epsilon)$ from $P$ to $Q$ where $Q = (I, S, m, \max)$.
%%   Let $x \in \Sigma^*$ and $y \in S(f(x))$.
%%   If $m(f(x), y) \geq \frac{1}{1 + \epsilon} \cdot m^*(f(x))$ and $m^*(f(x)) \geq c(x)$, then $x \in P$.
%% \end{lemma}
%% \begin{proof}
%%   Since $m^*(f(x)) \geq m(f(x), y) \geq \frac{1}{1 + \epsilon} m^*(f(x)) \geq \frac{1}{1 + \epsilon} c(x)$, we have $x \in P$ by condition 2 of the gap-introducing reduction.
%% \end{proof}
...
\begin{todo}
  Maybe check out \cite{ap06} for more another description of the completeness of \textsc{Maximum 3-Satisfiability}.
\end{todo}

\begin{todo}
  In \cite[Theorem~4]{kmsv99}, which they attribute to \cite{pcp}, the authors state directly that there is an enhanced gap-introducing reduction from any language in \NP{} to \textsc{Maximum 3-Satisfiability}.
\end{todo}
...
\begin{proof}
  ...
  \end{equation*}
  %% Here, there is some slack in the proof.
