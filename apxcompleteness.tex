...
\begin{proof}
  ...
  Since we have an enhanced gap-introducing reduction from $P$ to $Q$, we know $(x, g(x, g'(f(x), y_T))) \in R_P$ if and only if $x \in P$.
...
\begin{proof}[Proof of \autoref{thm:maxcomplete}]
  ...
  \begin{todo}
    Should this be ``maximum number of satisfiable clauses'' or ``total number of clauses'', or something else?
    Check the book, and revise the definitions of ``enhanced'' reductions.
  \end{todo}
