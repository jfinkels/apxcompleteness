...
\begin{proof}
  ...
  Suppose $y_T \in S_T(f'(f(x)))$ and $m_T(f'(f(x)), y_T) \geq \frac{1}{1 + \tilde{\epsilon}} \cdot m^*_T(f'(f(x))) = \frac{1}{1 + \beta} \cdot m^*_T(f'(f(x)))$.
  Our goal is to show that $(x, \tilde{g}(x, y_T)) \in R_P$ if and only if $x \in P$.
  Since we have an enhanced gap-preserving reduction from $Q$ to $T$, we know that $g'(f(x), y_T) \in S_Q(f(x))$ and $m_Q(f(x), g'(f(x), y_T)) \geq \frac{1}{1 + \alpha} \cdot m^*_Q(f(x))$.
  Since we have an enhanced gap-introducing reduction from $P$ to $Q$, we know that $(x, g(x, g'(f(x), y_T))) \in R_P$ if and only if $x \in P$.
