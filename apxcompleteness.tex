...
\begin{proofidea}
  ...
  For each subinterval we will use the Cook-Levin transformation to construct a Boolean formula which is satisfiable if and only if there is an approximate solution in that subinterval, and which exposes the approximate solution.
...
\begin{proof}[Proof of \autoref{thm:maxcomplete}]
  ...
  \end{equation}
  since it $a(x)$ is the measure of a $r_P$-approximate solution to a maximization problem.
  ...
  (Notice that the sizes of these subintervals are not necessarily equal; except the final subinterval, they are only non-decreasing.
  ...
  If we were to find a solution $y$ in, for example, interval $j$, then we would have
  \begin{equation*}
    {r_n}^j \cdot a(x) \leq m_P(x, y) \leq {r_n}^{j + 1} \cdot a(x),
  \end{equation*}
  and hence
  \begin{equation*}
    R_P(x, y) = \frac{m_P^*(x)}{m_P(x, y)} \leq r_n.
  \end{equation*}
  So this is our goal when constructing $f$ and $g$.
  \begin{todo}
    How can we get that last inequality?
  \end{todo}
  ...
  To define $g$, suppose $x \in I$ and $\tau \in S_Q(f(x, r))$.
  Let $\phi_i = C(B(x, i))$, let $\psi_i = f_s(\phi_i)$, and let $\psi = f(x, r)$, so $\tau$ is a satisfying assignment to $\psi$.
  Suppose $g_s$ is the special gap-extracting function described in the preceding discussion.
  Now for any $\tau$ satisfying a $1 - \epsilon$ factor of the maximum number of satisfiable clauses of $\psi$ and for each $i \in \{0, 1, \dotsc, k - 1\}$, if $\tau_i = g_s(\phi_i, \tau)$ then $\tau_i$ satisfies $\phi_i$ if and only if $\phi_i$ is satisfiable.
  We will assume for now that $\tau$ satisfies $1 - \epsilon$ of the maximum number of satisfiable clauses, and show that it is true later.
  There exists a maximum $i$ such that $\tau_i$ satisfies $\phi_i$; call it $j$.
  This implies
  \begin{equation*}
    {r_n}^j \cdot a(x) \leq m^*_P(x) \leq {r_n}^{j + 1} \cdot a(x),
  \end{equation*}

  Now we define $g$ as follows on input $x$, satisfying assignment $\tau$, and approximation factor $r$:
\end{proof}
