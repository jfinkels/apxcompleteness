...
\begin{proof}
  ...
  To compute the measure function, evaluate each of the functions and then compute the total number of those that are satisfied.
...
\begin{lemma}\label{lem:maxsatnnco}
  $\textsc{Maximum 3-Satisfiability} \in \NNCO$, and more generally in $\NPO$.
\end{lemma}
...
Finally, we remark that $\NC = \NNC = \NP$ implies $\P = \NP$, so it should be considered a more severe consequence
