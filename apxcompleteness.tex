...
\begin{proof}
  ...
  \begin{align*}
    ...
    g(\pair{U}{F}, \tau) & = \tau|_U \\
    c_P(\pair{U}{F}) & = M \\
    c_Q(\phi) & = M \cdot 2^k \cdot (k - 2) \\
    %% TODO change the definition of gap-preserving reduction to use 1 / 1 +
    %% \alpha and 1 / 1 + beta instead of just alpha and beta so that it better
    %% matches the definition of gap-introducing reduction
    \alpha(n + M) & = \frac{1}{2} \\
    \beta(n + M) & = ???
  \end{align*}
  where $n = |U|$, $M = |F|$, and $\tau|_U$ represents the restriction of the satisfying truth assignment $\tau$ to only the variables of $U$.
  ...
  Suppose $m^*_P(\pair{U}{F}) \geq c_P(x) = M$.
  Then $m^*_Q(f(\pair{U}{F})) = m^*_Q(f_0(\pair{U}{F})) = M \cdot 2^k \cdot (k - 2) = c_Q(f_0(\pair{U}{F}))$.
  \begin{todo}
...
\begin{lemma}\label{lem:gap3}
  \begin{todo}
    Update this to have the parameters from the enhanced reductions.
  \end{todo}
