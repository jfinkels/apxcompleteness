...
\item for all $x \in \Sigma^*$, we have $x \in L$ if and only if there exists a $y$ such that $(x, y) \in R$.
\end{enumerate}
(An equivalent definition for \NNC{} is given in \cite{wolf94}, where it is called \NNCpoly.)
...
\begin{lemma}[{\cite[Example~6.5]{book}}]\label{lem:three}
  There is a logarithmic space computable function $T$ that maps a Boolean formula in conjunctive normal form with $c$ clauses and at most $k$ literals per clause into an equivalent Boolean formula in conjunctive normal form with $c \cdot (k - 2)$ clauses and at most three literals per clause.
\end{lemma}
...
\begin{todo}
  Need to define \NNC-completeness.
\end{todo}

\begin{theorem}[\NC{} version of \autoref{thm:gap}]\label{thm:ncgap}
  Let $P$ be an \NNC-complete decision problem and $Q$ be a maximization problem in \NNCO.
...
\begin{corollary}\label{cor:notinncas}
  Let $P$ be an \NNC-complete decision problem and $Q$ be a maximization problem in \NNCO.
  If there is an \NC{} gap-introducing reduction from $P$ to $Q$ with gap parameter $\epsilon$ then $Q \notin \NCAS$ unless $\NC = \NNC$.
\end{corollary}
\begin{proof}
...
\begin{todo}
  Need to show that \textsc{Satisfiability} is \NNC-complete.
\end{todo}

\begin{todo}
  Need to show $\textsc{Maximum \textit{k}-Satisfiability} \in \NNCO$.
\end{todo}

\begin{theorem}[\NC{} version of \autoref{thm:hard}]\label{thm:nchard}
...
\end{theorem}
\begin{proof}
  Follows from \autoref{lem:gap3} and \autoref{thm:ncgap}.
\end{proof}
...
\end{corollary}
\begin{proof}
  Follows from \autoref{thm:nchard} and \autoref{cor:notinncas}.
\end{proof}
...
This means that if there were a polynomial time approximation scheme for this problem, then there would be one for all problems in \APX, and if there were a \NC{} approximation scheme for it, then there would be one for all problems in \NCX.
...
\begin{theorem}\label{thm:inncx}\label{thm:inapx}
  ...
  More generally, $\textsc{Maximum 3-Satisfiability} \in \APX$.
\end{theorem}
\begin{proof}
...
\begin{lemma}\label{lem:ncinterval}
  Let $P \in \NCX$, with $P = (I, S, m_P, \max)$, and let $A_P$ be the \FNC{} $r$-approximator for $P$, for some constant $r$.
  Let $r_n \geq 1$.
  There exists an \NNC{} circuit family, $\left\{B_\ell\right\}_{\ell \in \mathbb{N}}$, such that on input $(x, i)$, where $x \in I$ and $i \in \mathbb{N}$, the circuit $B_\ell$, where $\ell = |x| + |i|$, decides if there exists a solution $y$ with
  \begin{equation*}
    {r_n}^i \cdot m_P(x, A_P(x)) \leq m_P(x, y) \leq {r_n}^j \cdot m_P(x, A_P(x)).
  \end{equation*}
  Furthermore, if a solution $y$ exists, the output of $B_\ell$ is augmented with gates which output the bits of $y$.
\end{lemma}
\begin{proof}
  Let $p$ be the polynomial which bounds the size of the solutions for an instance $x$ of size $\ell$.
  Define $B$ as in \autoref{alg:ncinterval}.
  \begin{algorithm}
    \caption{\NNC{} algorithm that decides if there is an approximate solution for instance $x$ of problem $P$ in interval $i$%
      \label{alg:ncinterval}}
    \begin{algorithmic}[1]
      \Require{$x \in I$, where $|x| = \ell$, and $i \geq 0$}
      \Statex{}
      \Function{$B_\ell$}{$x$, $i$}:
        \State{\textbf{guess} $y$ with $|y| \leq p(|x|)$}
        \State{$a \gets m_P(x, A_P(x))$}
    ...
    \end{algorithmic}
  \end{algorithm}
  Since all operations used during the execution of $B_\ell$ are in \FNC, $B_\ell$ can be implemented by an \FNC{} circuit.
  Since $B_\ell$ outputs 1 if and only if there exists a solution $y$ with the stated bounds, the conclusion follows.
\end{proof}

\begin{lemma}[\NC{} version of \autoref{lem:cooklevin}]\label{lem:nccooklevin}
  There exists an \FNC{} circuit family, $\left\{C_\ell\right\}_{\ell \in \mathbb{N}}$, in which each circuit takes two inputs, the description of a nondeterministic polynomial time Turing machine and a tuple of its arguments, and outputs a Boolean formula $\Phi$ whose variables are the bits of the arguments to $C_\ell$, such that $C_\ell$ outputs 1 if and only if $\Phi$ is satisfiable.
  That is, $C_\ell(x, i)$ outputs 1 if and only if $\Phi$ is satisfiable.
\end{lemma}

\begin{todo}
  Get the notation and statement of the above lemma correct.
\end{todo}
...
\end{theorem}
\begin{proof}
  For the sake of brevity, we let $Q$ represent $\textsc{Maximum 3-Satisfiability}$.

  Let $P$ be a maximization problem in \NCX, so we have $P = (I, S, m, \max)$.
  Let $A_P$ be the \NC{} computable $r_P$-approximator for $P$, where $r_P$ is some constant.
  We need to define an \NC{} AP reduction $(f, g, \alpha)$ from $P$ to $Q$.
  We will delay the definition of the constant $\alpha$ until later in the proof, and until then we assume it has been fixed.
  We now wish to define \FNC{} functions $f$ and $g$ for all $r > 1$.
  Let $r_n = 1 + \alpha \cdot (r - 1)$; this is the desired upper bound of the performance ratio $R_P(x, g(x, \tau, r))$ when $R_Q(f(x, r), \tau) \leq r$, as required by the AP condition.
  ...
  This won't be a problem, as we only need a multiplicative constant approximation, not an additive constant approximation.)
  By construction, we have ${r_n}^k = {r_n}^{\ceil{\log_{r_n}{r_P}}} \geq {r_n}^{\log_{r_n}{r_P}} = r_P$, and hence $r_P \cdot a(x) \leq {r_n}^k \cdot a(x)$.
  Combining this with \autoref{eq:ncbounds}, we have $a(x) \leq m^*_P(x) \leq r_P \cdot a(x) \leq {r_n}^k \cdot a(x)$, so the optimal measure of $x$ must exist in one of these intervals.
  Now we construct $f$ and $g$ in such a way that for any $x \in I$, we can recover an approximate solution $y$ that lies in the same interval in which $m_P^*(x)$ lies.
  If we were to find a solution $y$ in the same interval in which the optimal measure lies, say, interval $j$, then we would have
  \begin{equation}\label{eq:ncsubinterval}
    {r_n}^j \cdot a(x) \leq m_P(x, y) \leq m^*_P(x) \leq {r_n}^{j + 1} \cdot a(x).
  \end{equation}
  Then, since
  \begin{equation*}
  ...
  \end{equation*}
  we find that
  \begin{equation}\label{eq:ncresult}
    R_P(x, y) = \frac{m_P^*(x)}{m_P(x, y)} = \frac{\frac{m^*_P(x)}{a(x)}}{\frac{m_P(x, y)}{a(x)}} \leq \frac{{r_n}^{j + 1}}{{r_n}^j} = r_n.
  \end{equation}
  So this is our goal when constructing $f$ and $g$.

  To define $f$ we need some auxiliary functions.
  Let $\mathcal{B}$ be the \NC{} circuit family guaranteed by \autoref{lem:ncinterval}.
  Let $f_s$ be the function from the enhanced gap-introducing reduction of \autoref{lem:gap3}.
  Let $\left\{C_\ell\right\}_{\ell \in \mathbb{N}}$ be the \FNC{} circuit family computing the Cook-Levin transformation, from \autoref{lem:cooklevin}.
  Now we can define $f$ by
  \begin{displaymath}
    f(x, r) = \bigwedge_{i = 0}^{k - 1}{f_s(C_{n_2}(B_{n_1}, (x, i)))},
  \end{displaymath}
  for all $x \in I$, where $n_1$ is the length of the encoding of $(x, i)$ and $n_2$ is the length of the encoding of $B_{n_1}$ along with $(x, i)$.
  Let us explain this function.
  Since $B_{n_1}$ outputs a solution $y$ if it outputs 1, $C_{n_2}(B_{n_1}, (x, i))$ will have some gates, $y_1, y_2, \dotsc, y_{p(|x|)}$, which, correspond to the bits of a solution $y$, should one exist in subinterval $i$.
  Applying $f_s$ to this Boolean formula produces a 3-CNF formula which has a gap behavior which will be discussed later (since it depends on $g_s$).
  Observe that $f(x, r)$ is still a 3-CNF formula, since a conjunction of 3-CNF formulae is itself a 3-CNF formula.
  We assume without loss of generality also that each formula $f_s(C_{n_2}(B_{n_1}, (x, i)))$ has the same number of clauses, say $M$.
  \begin{todo}
    Why is this without loss of generality?
    Adding more clauses may change the fraction of satisfiable clauses\ldots
  \end{todo}
  \begin{todo}
    Explain why $f$ is in \FNC.
  \end{todo}

  Now, to determine how to define $g$, first suppose $x \in I$ and $\tau \in S_Q(f(x, r))$.
  For each $i$ with $0 \leq i \leq k - 1$, let $\phi_i = C_{n_2}(B_{n_1}, (x, i))$ and $\psi_i = f_s(\phi_i)$.
  Let $\psi = f(x, r) = \bigwedge_i{\psi_i}$, so $\tau$ is a truth assignment to $\psi$ which satisfies some of the clauses.
  Suppose $g_s$ is the \FNC{} function from the \NC{} enhanced gap-introducing reduction which produces solutions for $P$, as described in the preceding discussion.
  For any $\tau$ that satisfies a $\frac{1}{1 + \epsilon}$ factor of the maximum number of satisfiable clauses in $\psi_i$ for all $i \leq k - 1$, if we let $\tau_i = g_s(\phi_i, \tau)$ then $\tau_i$ satisfies $\phi_i$ if and only if $\phi_i$ is satisfiable.
  We will assume for now that $\tau$ does indeed satisfy a $\frac{1}{1 + \epsilon}$ factor of the maximum number of satisfiable clauses of $\psi_i$ for all $i$, and delay the proof of this fact until later.
  Furthermore, if $\phi_i$ is satisfiable via truth assignment $\tau_i$, then we can recover the bits of the solution $y$ from the appropriate variables of $\phi_i$, as described previously.
  Let $T$ be the function which outputs $y$ given $\phi_i$ and $\tau_i$.
  \begin{todo}
    Note that $T$ is a \FNC{} circuit.
  \end{todo}
  ...
  \begin{equation*}
    {r_n}^j \cdot a(x) \leq m_P(x, y) \leq m^*_P(x) \leq {r_n}^{j + 1} \cdot a(x),
  \end{equation*}
  which is exactly the statement of \autoref{eq:ncsubinterval}, and hence \autoref{eq:ncresult} follows.
  Therefore, we define $g(x, \tau, r)$ as in \autoref{alg:ncg}.
  Observe that $g$ is computable by an \FNC{} circuit family since $k$, the number of intervals to search, is a constant, and all other functions are \NC{} computable.
  \begin{algorithm}
    \caption{\NC{} algorithm that computes a $r_n$-approximate solution for $x$%
      \label{alg:ncg}}
    \begin{algorithmic}
      \Require{$x \in I$, a truth assignment $\tau$, $r > 1$}
      \Statex{}
...
\begin{theorem}[{\cite[Theorem~8.7]{book}}]\label{thm:ncminmax}
  For every minimization problem $P \in \NCX$, where $P = (I, S, m_P, \min)$, there exists a maximization problem $Q \in \NCX$ such that $P \NCAPr Q$.
\end{theorem}
