...
\begin{definition}[\textsc{$k$-Function Satisfiability}]
  \mbox{}
  \begin{description}
  \item[Input:] finite set of Boolean variables $x_1, x_2, \ldots, x_n$ and a finite set of  functions $g_1, g_2, \ldots, g_m$, each of which is a function of $k$ of the variables.
  \item[Question:] Does there exist a truth assignment $\tau$ such that all the functions are satisfied?
  \end{description}
\end{definition}
...
\begin{definition}[\textsc{Maximum $k$-Function Satisfiability}]
  ...
  \item[Input:] finite set of Boolean variables $x_1, x_2, \ldots, x_n$ and a finite set of  functions $g_1, g_2, \ldots, g_m$, each of which is a function of $k$ of the variables.
  \item[Solution:] a truth assignment $\tau$ to the variables.
  \item[Measure:] The number of satisfied functions.
  \end{description}
\end{definition}
...
\begin{lemma}
  Let $P$ be an optimization problem in \NPO.
  If there exists a constant $\epsilon$ such that for all $r < 1 + \epsilon$, no polynomial time $r$-approximator exists unless $\P = \NP$, then $P \notin \PTAS$ unless $\P = \NP$.
\end{lemma}
\begin{proof}
  \begin{todo}
    Fill me in (this is simple).
  \end{todo}
\end{proof}

\begin{theorem}[{\cite[Theorem~6.3]{book}} and {\cite[Corollary~29.8]{vazirani}}]
